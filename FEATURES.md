...
| ST7920 128×64 display menu (`espresso_machine_display`) | 🚧 | **BETA** — Full action-hub UI — Home Screen (idle + brew/steam/grind active), Brew/Steam/Grinder/Settings/Maintenance menus, number-edit mode, confirm dialogs, error overlays, 4 themes (classic/minimal/barista/dark), screensaver. Wired via `display_id/encoder_id/button_id`; adapts automatically to optional grinder/heater/flow-meter bindings; opt-in only (not included in default examples); see `tests/test_display.yaml` |
