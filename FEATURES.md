...
| `espresso_machine_vizualise` | `component` | 🚧 | **BETA** — visualizer.coffee shot upload; not fully verified end-to-end; opt-in only; accepts `espresso_machine:` reference for auto-recording during brew; visualizer.coffee parallel-arrays JSON format; configurable server URL, profile name, machine name, and Bearer token auth; exponential-backoff retry; virtual `http_post()` for test mocking; see `docs/vizualise.md` |
